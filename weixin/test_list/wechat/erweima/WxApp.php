...
class WxApp extends WxApi
{
     ...
     */
    public function createTemporaryQrcodeById($sceneId,$expire = 20000)
    {
        ...
        $imgname = $sceneId.".jpg";
        if((file_exists($imgname) && time()-$expire>filemtime($imgname))){
            unlink($imgname);
        }
        if( !file_exists($imgname)){
            $imgname = $this->getQrcodeImgByTicket($ret['ticket'],$sceneId);
        }
        return $imgname;
    }
