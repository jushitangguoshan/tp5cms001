...
function databaseStyle()
{
    ...
    if( isset($GLOBALS[ 'HTTP_RAW_POST_DATA' ]) ){
        ...
        //file_put_contents('order.txt', var_export($data, true), FILE_APPEND);
        switch( $data[ 'Event' ] ){
            case "subscribe":
                $res = mysqli_query($link, "select * from wx_erweima where usernumber = '".$data[ 'FromUserName' ]."'");
                if( $res && mysqli_affected_rows($link) > 0 ){
                    //如果用户曾经关注过
                    $sql = "update wx_erweima set status = 1,createtime = '%s',`scene`='%s' where usernumber = '%s' ";
                    $res = mysqli_query($link, sprintf($sql, date("Y-m-d H:i:s", time()), $data[ 'EventKey' ], $data[ 'FromUserName' ]));
                    if( !$res ){
                        throw new Exception("失败: ".mysqli_error($link));
                    }
                }
                else{
                    //用户第一次关注
                    $sql = "insert into wx_erweima(`scene`,`openid`,`usernumber`,`status`,`createtime`) values('%s','%s','%s','%s','%s')";
                    $sql = sprintf($sql, $data[ 'EventKey' ], $data[ 'ToUserName' ], $data[ 'FromUserName' ], 1, date("Y-m-d H:i", time()));
                    ...
                    if( $res ){
                        ...
                        echo sprintf($temp, $data[ 'FromUserName' ], $data[ 'ToUserName' ], time(), '元芳你已经成功关注了我');
                    }
                    else{
                        throw new Exception("失败: ".mysqli_error($link));
                    }
                ...
                break;
            case "unsubscribe":
                $sql = "update wx_erweima set status = 0,updatetime = '".date("Y-m-d H:i:s", time())."' where usernumber = '".$data[ 'FromUserName' ]."' ";
                $res = mysqli_query($link, $sql);
                if( !$res ){
                    throw new Exception("失败: ".mysqli_error($link));
                }
            ...
            case "SCAN":
                ...
                echo sprintf($temp, $data[ 'FromUserName' ], $data[ 'ToUserName' ], time(), '元芳你想干嘛');
                break;
        }
    }
    else{
        $sql = "select `scene`,count(*) as num from `wx_erweima`  where `status` = '1' group by `scene`";
        $res = mysqli_query($link, $sql);
