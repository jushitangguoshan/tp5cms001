		...
		<table class="table we7-table table-hover vertical-middle table-manage">
			...
			<tr ng-repeat="module in moduleList">
				<td class="text-left module-img">
					<img ng-if="module.main_module == ''" ng-src="{{ module.logo }}" class="img-responsive module-img icon"/>
...
<div class="js-system-module-detail" ng-controller="detailCtrl" ng-cloak>
	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && (upgradeInfo.from == 'local' || !upgradeInfo)">
		<col width="300px"/>
		<col />
		<col width="200px;"/>
		<tr>
			<th ng-show="upgradeInfo.from != 'local' || !upgradeInfo">分支名称</th>
			<th ng-show="upgradeInfo.from != 'local' || !upgradeInfo">分支价格</th>
			<th>目前版本</th>
			<th>最新版本</th>
			<th class="text-right">操作</th>
		</tr>
		<tr>
			<td ng-show="upgradeInfo.from != 'local' || !upgradeInfo">{{ branchname }}</td>
			<td ng-show="upgradeInfo.from != 'local' || !upgradeInfo"><span class="label label-success">当前分支</span></td>
			<td>{{ moduleinfo.version }}</td>
			<td>{{ upgradeInfo ? upgradeInfo.best_version : '暂无最新版本' }}</td>
			<td class="text-right">
				<?php  if(permission_check_account_user('see_module_manage_system_ugrade')) { ?>
				<a ng-if="upgradeInfo" href="<?php  echo url('module/manage-system/upgrade')?>module_name={{ moduleinfo.name }}" class="btn btn-danger">升级</a>
				<button ng-if="!upgradeInfo" href="#" ng-click="getUpgradeInfo(1)" class="btn btn-primary">{{ getUpgradeInfoButton }}</button>
				<?php  } ?>
			</td>
		</tr>
	</table>
