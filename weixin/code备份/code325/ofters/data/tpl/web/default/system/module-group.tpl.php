...
<div id="js-module-group" class="module-group-page clearfix" ng-controller="moduleGroup" ng-cloak>
	<img src="<?php  echo tomedia($module['logo'])?>" class="module-img" alt=""><?php  if(!empty($module['title'])) { ?><?php  echo $module['title'];?><?php  } else { ?>模板<?php  } ?>
	<div class="we7-header">
		分配应用权限组(这里来定义哪些应用权限组可使用<?php  if(!empty($module['title'])) { ?><?php  echo $module['title'];?> 功能<?php  } else { ?>该模板<?php  } ?>)
	</div>
