...
<ul class="we7-step">
	<li class="new <?php  if($do == 'base_info') { ?>active<?php  } ?>"> <span class="content">1. 设置<?php  echo $account_all_type_sign[$sign]['title'];?>信息</span> </li>
	<?php  if(!empty($_W['isfounder'])) { ?>
	<li class="new<?php  if($do == 'account_modules') { ?>active<?php  } ?>"> <span class="content">2. 设置权限</span></li>
	<?php  } ?>
	<?php  if(in_array($sign, array(ACCOUNT_TYPE_SIGN, XZAPP_TYPE_SIGN))) { ?>
	<li class="new<?php  if($do == 'account_help') { ?>active<?php  } ?>"><span class="content"><?php  if(empty($_W['isfounder'])) { ?>2<?php  } else { ?>3<?php  } ?> 引导页面</span></li>
	...
	<li class="new <?php  if($do == 'create_version') { ?>active<?php  } ?>"><span class="content"><?php  if(empty($_W['isfounder'])) { ?>2<?php  } else { ?>3<?php  } ?> 新建版本</span></li>
	<?php  } ?>
</ul>
	...
	<div id="miniapp_base_info">
				...
				<div class="form-defalut we7-form">
					<div class="form-group">
						...
						<div class="form-controls col-sm-10">
							<input type="text" name="name" ng-model="miniapp.name" class="form-control wxapp-name" placeholder="<?php  echo $account_all_type[$type]['title'];?>名称" required>
						</div>
					</div>
					<div class="form-group">
		...
		<script>
			angular.module('wxApp').value('config', {
				'miniapp' : {'name':''},
				'preset_type_info' : {"type_sign": "<?php  echo $sign;?>"},
			});
...
<form action="" method="post" class="we7-form account-create-step" id="js-account-post-step3" ng-controller="AccountPostStepThree" ng-cloak>
	...
	<div class="col-sm-offset-2">
		<input type="hidden" name="token" value="<?php  echo $_W['token'];?>">
		<input type="submit" name="submit" value="<?php  if(in_array($sign, array(WEBAPP_TYPE_SIGN))) { ?>完成<?php  } else { ?>下一步<?php  } ?>" class="btn btn-primary btn-submit"/>
	</div>
</form>
<script type="text/javascript">
	angular.module('accountApp').value('config', {
