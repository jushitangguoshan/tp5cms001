...
<?php  if($m == 'keyword' || $m == 'userapi' || !in_array($m, $sysmods)) { ?>
	<div id="js-keyword-display" ng-controller="KeywordDisplay" ng-cloak>
		<div class="search-box we7-margin-bottom">
			...
			<?php  if(in_array($m, $sysmods) && $m != 'userapi') { ?><a href="<?php  echo url('platform/reply/post', array('m' => 'apply'));?>" class="btn btn-default  we7-margin-left">添加应用关键字</a><?php  } ?>
		</div>
	...
	<script>
		...
		$(function () {
			...
			angular.module('replyFormApp').value('config', {
				...
				'replyUrl': '<?php  echo url('platform/reply', array('m' => 'keyword'));?>',
			});
				...
				<tr>
					...
					<td class="text-left">
						<?php  if(!empty($setting[$name]['type'])) { ?>
							...
							<?php  } else { ?>
								<?php  echo $module[$setting[$name]['module']]['title'];?>
							<?php  } ?>
						<?php  } else { ?>
